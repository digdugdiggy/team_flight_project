...
teamFlight.robot({
	...
	work: function(droneObj) {
		...
		//droneObj.drone.config('general:navdata_demo', 'FALSE');
		//droneObj.drone.config('general:emergencyLanding', 'TRUE');
	
		var express = require('express')
			, app = express()
			, server = require("http").createServer(app)

		//Used for video stream (port 6400)
		...
		if (commandMode == 0){
			var server = net.createServer(function (commandReceiver){
				...
				commandReceiver.on('data', function (moveCommand) {
					...
					if (moveCommand == "disableEmergency"){
						droneObj.disableEmergency(droneObj);
					}
					if (moveCommand == "droneCalibrate"){
						droneObj.droneCalibrate(droneObj);
					}
		...
		/*
		droneObj.nav.on('navdata', function(data3) {
			console.log(data3);
		})*/
	},
	...
	droneCalibrate: function(droneRef) {
		droneRef.drone.calibrate(0);
		console.log("     " + dispText + "Calibrating");
	},  
	
	disableEmergency: function(droneRef) {
		droneRef.drone.disableEmergency();
		console.log("     " + dispText + "Disable Emergency");
	},  
